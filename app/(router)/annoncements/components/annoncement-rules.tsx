...
interface AnnoncementRulesProps {
  annoncement: Annoncement | null;
}

function AnnoncementRules({ annoncement }: AnnoncementRulesProps) {
  const rulesIcons = [
    {
  ...
  return (
    <div className="w-full rounded-xl bg-white flex flex-col gap-2 text-slate-900 h-full px-4 py-3">
      ...
      {annoncement?.additionalFilters.some(
        (filteredItem) => filteredItem.value === "Взимается залог"
      ) && (
